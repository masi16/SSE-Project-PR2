...
from services import resolucion as resolucion_service
...
async def create_resolucion(
    ...
    if not created:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No se pudo crear la resolución")
    return created
...
async def update_resolucion(
    ...
    if not updated:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Resolución no encontrada o no autorizada")
    return updated
