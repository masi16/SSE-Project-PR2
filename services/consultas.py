...
from schemas.consultas import ConsultaCreate, ConsultaOut
...
async def create_consulta(consulta: ConsultaCreate) -> ConsultaOut:
    ...
    """)
    row = await get_db().fetch_one(query, values=consulta.dict())
    return ConsultaOut(**row)

async def get_consultas() -> List[ConsultaOut]:
    query = text("SELECT id, cliente_id, asunto, descripcion, fecha FROM consultas")
...
async def get_consulta(consulta_id: int) -> ConsultaOut:
    query = text("SELECT id, cliente_id, asunto, descripcion, fecha FROM consultas WHERE id = :id")
    row = await get_db().fetch_one(query, values={"id": consulta_id})
    if row:
        return ConsultaOut(**row)
...
async def delete_consulta(consulta_id: int) -> None:
    query = text("DELETE FROM consultas WHERE id = :id")
    result = await get_db().execute(query, values={"id": consulta_id})
