...
from schemas.movimientos import ResolucionCreate, ResolucionOut 

async def create_resolucion(resolucion: ResolucionCreate) -> ResolucionOut:
    new_resolucion = await db.resoluciones.insert_one(resolucion.dict())
    created_resolucion = await db.resoluciones.find_one({"_id": new_resolucion.inserted_id})
    return ResolucionOut(**created_resolucion)

async def get_resolucion_by_id(resolucion_id: str) -> ResolucionOut:
    resolucion = await db.resoluciones.find_one({"_id": resolucion_id})
...
async def delete_resolucion(resolucion_id: str) -> None:
    result = await db.resoluciones.delete_one({"_id": resolucion_id})
