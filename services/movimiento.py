...
async def create_movimiento(movimiento: MovimientoCreate) -> MovimientoOut:
    new_movimiento = await get_db().movimientos.insert_one(movimiento.dict())
    created_movimiento = await get_db().movimientos.find_one({"_id": new_movimiento.inserted_id})
    return MovimientoOut(**created_movimiento)

async def get_movimiento_by_id(movimiento_id: str) -> MovimientoOut:
    movimiento = await get_db().movimientos.find_one({"_id": movimiento_id})
...
async def delete_movimiento(movimiento_id: str) -> None:
    result = await get_db().movimientos.delete_one({"_id": movimiento_id})
